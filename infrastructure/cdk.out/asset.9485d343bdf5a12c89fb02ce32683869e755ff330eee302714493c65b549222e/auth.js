...
AWS.config.update({
    ...
    credentials: null  // Use anonymous credentials for Cognito operations
});
...
function validatePassword(password) {
    ...
    if (failed.length > 0) {
        const readableReqs = failed.map(req => {
            switch(req) {
        ...
        throw new Error(`Password must contain ${readableReqs.join(', ')}`);
    }
...
function updatePasswordStrength(password) {
    const requirements = {
        ...
        'uppercase': { met: /[A-Z]/.test(password), text: 'Uppercase letter' },
        'lowercase': { met: /[a-z]/.test(password), text: 'Lowercase letter' },
        'number': { met: /[0-9]/.test(password), text: 'Number' },
        'special': { met: /[^A-Za-z0-9]/.test(password), text: 'Special character' }
    };
    ...
    if (!strengthIndicator) {
        ...
        document.getElementById('registerPassword').parentNode.appendChild(strengthIndicator);
    }

    strengthIndicator.innerHTML = Object.entries(requirements)
        .map(([key, { met, text }]) => `
            <div style="display: flex; align-items: center; margin: 2px 0;">
                <span style="color: ${met ? '#4CAF50' : '#FF5252'}">
                    ${met ? '✓' : '✗'}
                </span>
...
registerForm.addEventListener('submit', async (e) => {
    ...
    try {
            ...
            userPool.signUp(email, password, attributeList, null, (err, result) => {
                if (err) {
                    ...
                    if (err.code === 'UsernameExistsException') {
                        ...
                        reject(new Error('Invalid email format or password requirements not met.'));
                    } else if (err.code === 'InvalidPasswordException') {
                        reject(new Error('Password does not meet requirements. Please ensure it contains uppercase, lowercase, numbers, and special characters.'));
                    } else {
                        reject(err);
                    }
                    return;
        ...
        alert(error.message || getGenericErrorMessage(error));
    }
});
