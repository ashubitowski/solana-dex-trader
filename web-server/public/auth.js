    ...
    if (cognitoUser != null) {
        try {
            ...
            if (session && session.isValid()) {
                ...
                if (typeof window.initializeAppUI === 'function') {
                    window.initializeAppUI(); // Call the function exposed by app.js
                } else {
                    logAuthEvent('Check', 'Warning: initializeAppUI function not found in app.js');
                }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loginErrorDiv = document.getElementById('loginError'); // For specific error handling
    ...
    const requiredElements = {
        ...
        resetPasswordForm, newPasswordForm, backToLogin, backToReset, verifyForm,
        verifyEmailInput, verifyCodeInput, resendCodeLink, registerPasswordInput, 
        passwordStrengthIndicator // passwordStrengthText is optional
    };
    ...
    if (registerPasswordInput) { // Check if element exists before adding listener
    ...
    registerForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            logAuthEvent('Register', 'Registration successful, showing verification form', { username: result.user.getUsername() });
            // Instead of showing success message/toggling, show verification form
            registerForm.style.display = 'none';
            verifyForm.style.display = 'block';
            verifyEmailInput.value = email; // Pre-fill email
            showAuthMessage('success', 'Registration successful! Check email for verification code.', 'verifyForm');
        } catch (error) {
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (error.code === 'UserNotConfirmedException') {
                // Show verification form instead of just an error
                logAuthEvent('Login', 'User not confirmed, showing verification form');
                loginForm.style.display = 'none';
                verifyForm.style.display = 'block';
                verifyEmailInput.value = email; // Pre-fill email
                showAuthMessage('error', 'Account not verified. Please enter the code sent to your email.', 'verifyForm');
            } else {
    ...
    verifyForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        clearAuthMessages('verifyForm');
        logAuthEvent('Verify', 'Starting account verification process');
        const email = verifyEmailInput.value;
        const code = verifyCodeInput.value;
        if (!code || !email || !userPool) {
            showAuthMessage('error', 'Missing email or code.', 'verifyForm');
            return;
        }
        // Need to create the CognitoUser object to call confirmRegistration
        const userData = { Username: email, Pool: userPool };
        const cognitoUser = new AmazonCognitoIdentity.CognitoUser(userData);
        try {
            await new Promise((resolve, reject) => {
                // Call confirmRegistration on the cognitoUser object
                cognitoUser.confirmRegistration(code, true, (err, result) => { // true = forceAliasCreation
                    if (err) { reject(err); return; }
                    resolve(result);
                });
            });
            logAuthEvent('Verify', 'Account verification successful');
            showAuthMessage('success', 'Account verified successfully! Please login.', 'verifyForm');
            // Show login form after a delay
            setTimeout(() => {
            ...
            logAuthEvent('Verify', 'Account verification failed', error);
            showAuthMessage('error', getGenericErrorMessage(error), 'verifyForm');
        }
    });
    ...
    resendCodeLink.addEventListener('click', async (e) => {
        e.preventDefault();
        clearAuthMessages('verifyForm');
        logAuthEvent('Verify', 'Resending verification code');
        const email = verifyEmailInput.value;
        if (!email || !userPool) {
             showAuthMessage('error', 'Email address is missing.', 'verifyForm');
             return;
        }
        const userData = { Username: email, Pool: userPool };
        const cognitoUser = new AmazonCognitoIdentity.CognitoUser(userData);
        try {
            ...
            logAuthEvent('Verify', 'Verification code resent successfully');
            showAuthMessage('success', 'Verification code resent. Check your email.', 'verifyForm');
        } catch (error) {
            logAuthEvent('Verify', 'Failed to resend verification code', error);
            showAuthMessage('error', getGenericErrorMessage(error), 'verifyForm');
        }
    });
